<template>
  ...
  >
    ...
    <div class="vc-sketch-controls">
      <div class="vc-sketch-color-wrap" :class="[disableAlpha && 'vc-disabled-opacity-sketch-color-wrap']">
        <div
    ...
    <div class="vc-sketch-presets" role="group" aria-label="A color preset, pick one to set as current color">
      ...
      <div class="vc-sketch-presets-list">
        <template v-for="c in slicePresetColors">
          <div
    ...
    <!--     自定义颜色-->
    <customColor :colors="colors" v-if="showCustomColors" :cacheColorKey="cacheColorKey" @handlePreset="handlePreset" />
...
import customColor from './common/CustomColor.vue'
...
export default {
  ...
  computed: {
    ...
    slicePresetColors({ presetColors, presetColorsCount }) {
      if (!presetColors) return []
      return presetColors.slice(0, presetColorsCount)
    }
  },
  methods: {
    handlePreset(c) {
...
.vc-disabled-opacity-sketch-color-wrap {
  width: 10px;
  height: 10px;
}
